...
export const algorithm: Flashcard[] = [
  ...
  },
  {
    id: 14,
    category: "알고리즘",
    question: "단일 연결 리스트(Single Linked List)가 뭔가요?",
    answer:
      "Single Linked List(단일 연결 리스트)는 각 노드가 데이터와 다음 노드를 가리키는 포인터(next)를 가진 선형 자료구조입니다.\n\n배열과 달리 인덱스가 없어 접근에 O(n)이 걸리지만, 맨 앞에 삽입/삭제하는 연산은 O(1)로 빠릅니다.\n\nhead에서 시작하여 한 방향으로만 순회할 수 있으며, 동적으로 크기가 변하는 데이터에 적합합니다.",
  },
  {
    id: 15,
    category: "알고리즘",
    question: "이중 연결 리스트(Double Linked List)가 뭔가요?",
    answer:
      "Double Linked List(이중 연결 리스트)는 각 노드가 이전 노드(prev)와 다음 노드(next) 두 개의 포인터를 가진 자료구조입니다.\n\n양방향 순회가 가능하여 특정 노드에서 앞뒤로 이동할 수 있고, 뒤에서부터 탐색이 가능하므로 탐색 시간을 절반으로 줄일 수 있습니다.\n\n단일 연결 리스트보다 메모리를 더 사용하지만, 삭제 연산 등에서 더 유연합니다.",
  },
  {
    id: 16,
    category: "알고리즘",
    question: "스택(Stack)이 뭔가요?",
    answer:
      "Stack(스택)은 후입선출(LIFO, Last In First Out) 원칙을 따르는 자료구조입니다.\n\n가장 마지막에 추가된 요소가 가장 먼저 제거되며, push(삽입)와 pop(제거) 연산이 모두 O(1)입니다.\n\n함수 호출 스택, 실행 취소(Undo) 기능, 괄호 유효성 검사, DFS 구현 등에 활용됩니다.",
  },
  {
    id: 17,
    category: "알고리즘",
    question: "큐(Queue)가 뭔가요?",
    answer:
      "Queue(큐)는 선입선출(FIFO, First In First Out) 원칙을 따르는 자료구조입니다.\n\n가장 먼저 추가된 요소가 가장 먼저 제거되며, enqueue(삽입)와 dequeue(제거) 연산이 모두 O(1)입니다.\n\nBFS(너비 우선 탐색), 작업 스케줄링, 프린터 대기열 등 순서가 중요한 처리에 활용됩니다.",
  },
  {
    id: 18,
    category: "알고리즘",
    question: "Binary Search Tree(이진 탐색 트리)가 뭔가요?",
    answer:
      "Binary Search Tree(이진 탐색 트리)는 각 노드가 최대 두 개의 자식을 가지며, 왼쪽 자식은 부모보다 작고 오른쪽 자식은 부모보다 큰 값을 저장하는 자료구조입니다.\n\n이 규칙 덕분에 탐색, 삽입, 삭제의 평균 시간 복잡도가 O(log n)입니다.\n\n그러나 편향 트리(한쪽으로 치우친 트리)가 되면 O(n)으로 성능이 저하될 수 있어, AVL 트리나 레드-블랙 트리 같은 균형 트리로 개선합니다.",
  },
  {
    id: 19,
    category: "알고리즘",
  ...
  },
  {
    id: 21,
    category: "알고리즘",
    question: "Binary Heap이 뭔가요?",
    answer:
      "Binary Heap(이진 힙)은 완전 이진 트리 형태의 자료구조로, 최대 힙(Max Heap)에서는 부모가 자식보다 항상 크고, 최소 힙(Min Heap)에서는 부모가 자식보다 항상 작습니다.\n\n삽입과 삭제 연산이 O(log n)이며, 최댓값 또는 최솟값 조회는 O(1)입니다.\n\n배열로 효율적으로 구현할 수 있으며, 우선순위 큐와 힙 정렬의 기반이 됩니다.",
  },
  {
    id: 22,
    category: "알고리즘",
    question: "Priority Queue(우선순위 큐)가 뭔가요?",
    answer:
      "Priority Queue(우선순위 큐)는 각 요소에 우선순위가 부여되어, 우선순위가 높은 요소가 먼저 처리되는 자료구조입니다.\n\n일반적으로 Binary Heap을 사용하여 구현하며, 삽입과 추출 연산이 O(log n)입니다.\n\n다익스트라 알고리즘, 작업 스케줄링, 허프만 코딩 등 우선순위 기반 처리가 필요한 곳에 널리 사용됩니다.",
  },
  {
    id: 23,
    category: "알고리즘",
    question: "Hash Table이 뭔가요?",
    answer:
      "Hash Table(해시 테이블)은 키를 해시 함수를 통해 인덱스로 변환하여 값을 저장하는 자료구조입니다.\n\n평균적으로 삽입, 삭제, 조회가 O(1)로 매우 빠릅니다.\n\n서로 다른 키가 같은 인덱스를 가리키는 해시 충돌이 발생할 수 있으며, 이를 체이닝(Chaining)이나 개방 주소법(Open Addressing)으로 해결합니다.",
  },
  {
    id: 24,
    category: "알고리즘",
    question: "Graph(그래프)가 뭔가요?",
    answer:
      "Graph(그래프)는 정점(Vertex)과 간선(Edge)으로 구성된 비선형 자료구조로, 데이터 간의 관계를 표현합니다.\n\n방향 그래프와 무방향 그래프, 가중치 그래프와 비가중치 그래프로 분류되며, 인접 행렬(Adjacency Matrix)이나 인접 리스트(Adjacency List)로 구현합니다.\n\nSNS 관계, 지도 경로, 네트워크 구조 등 다양한 현실 문제를 모델링하는 데 사용됩니다.",
  },
  {
    id: 25,
    category: "알고리즘",
