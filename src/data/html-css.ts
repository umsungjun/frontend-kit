...
export const htmlCss: Flashcard[] = [
  ...
  },
  {
    ...
    answer:
      "주소창에 `www.google.com`을 입력하면 URL을 해석하고, 서버를 찾아 연결한 다음, 데이터를 받아 브라우저가 렌더링하는 과정이 진행됩니다.\n\nURL 해석: 입력한 주소를 URL로 해석합니다. `https`는 프로토콜, `www.google.com`은 도메인입니다.\nDNS 조회: 도메인에 해당하는 IP 주소를 DNS로 찾습니다. 브라우저 캐시를 먼저 확인하고 없으면 DNS 서버에 질의합니다.\n서버 연결: TCP 3-way 핸드셰이크로 연결을 맺습니다. 클라이언트가 SYN, 서버가 SYN-ACK, 다시 클라이언트가 ACK를 보냅니다. HTTPS라면 TLS 핸드셰이크로 인증서를 교환하고 암호화 키를 협상합니다.\nHTTP 요청과 응답: GET 요청을 보내면 서버가 200 OK와 함께 HTML을 응답합니다.\n브라우저 렌더링: 응답받은 HTML로 화면을 그립니다. 파싱 중 만난 CSS, JavaScript, 이미지는 추가로 요청합니다.\n\nURL 해석 → DNS 조회 → 서버 연결 → HTTP 요청/응답 → 브라우저 렌더링 순서입니다.",
  },
  {
    id: 35,
    category: "HTML/CSS",
