...
export const react: Flashcard[] = [
  ...
  },
  {
    ...
    answer:
      "Virtual DOM은 실제 DOM의 가벼운 복사본으로, 메모리에 존재하는 자바스크립트 객체입니다.\n\n상태가 변경되면 새로운 Virtual DOM 트리를 생성하고 이전 트리와 비교(diffing)하여 변경된 부분만 실제 DOM에 반영(reconciliation)합니다.\n\n이를 통해 불필요한 DOM 조작을 최소화하여 렌더링 성능을 향상시킵니다.",
  },
  {
    id: 4,
    category: "React",
  ...
  },
  {
    ...
    answer:
      "props는 외부에서 주입되는 읽기 전용 데이터이고, state는 컴포넌트 자체가 소유하고 관리하는 변경 가능한 데이터입니다.\n\nprops는 부모 컴포넌트로부터 전달받는 읽기 전용 데이터로, 자식 컴포넌트에서 직접 수정할 수 없습니다.\n\nstate는 컴포넌트 내부에서 관리하는 변경 가능한 데이터로, setState나 useState를 통해 업데이트하면 리렌더링이 발생합니다.\n\n데이터의 소유 주체와 변경 가능 여부가 두 개념의 핵심적인 차이입니다.",
  },
  {
    id: 12,
    category: "React",
  ...
  },
  {
    ...
    answer:
      "리덕스는 예측 가능한 상태 관리를 위해 세 가지 기본 원칙을 따릅니다.\n\n첫째, 단일 스토어(Single Source of Truth)로 애플리케이션의 전체 상태를 하나의 스토어에 저장합니다.\n둘째, 상태는 읽기 전용(State is Read-only)으로 상태를 변경하려면 반드시 액션을 디스패치해야 합니다.\n셋째, 변경은 순수 함수로 이루어져야(Changes are made with Pure Functions) 하며, 리듀서는 이전 상태와 액션을 받아 새로운 상태를 반환하는 순수 함수여야 합니다.",
  },
  {
    id: 16,
    category: "React",
  ...
  },
  {
    ...
    answer:
      "useLayoutEffect는 useEffect와 동일한 시그니처를 가지지만, DOM이 업데이트된 직후 브라우저가 화면을 그리기(paint) 전에 동기적으로 실행됩니다.\n\nuseEffect는 화면이 그려진 후 비동기적으로 실행되는 반면, useLayoutEffect는 레이아웃 측정이나 DOM 조작이 필요한 경우에 사용하여 화면 깜빡임(flickering)을 방지할 수 있습니다.\n\n동기적으로 실행되므로 과도하게 사용하면 렌더링 성능에 영향을 줄 수 있어 필요한 경우에만 사용해야 합니다.",
  },
  {
    id: 31,
    category: "React",
  ...
  },
  {
    ...
    question: "SSR이 뭔가요?",
    answer:
      "SSR(Server-Side Rendering)은 서버에서 HTML을 완성하여 클라이언트에 전달하는 렌더링 방식입니다.\n\n서버가 요청을 받으면 리액트 컴포넌트를 HTML 문자열로 렌더링하여 응답하고, 클라이언트는 완성된 HTML을 받아 즉시 화면에 표시한 후 자바스크립트를 로드하여 인터랙션을 활성화합니다.\n\nSPA의 초기 로딩 속도와 SEO 문제를 해결할 수 있으며, Next.js가 대표적인 SSR 프레임워크입니다.",
  },
  {
    id: 36,
    category: "React",
  ...
  },
  {
    ...
    answer:
      "Controlled 패턴은 폼 요소의 값을 리액트의 state로 관리하는 방식입니다.\n\ninput의 value 속성에 state를 바인딩하고, onChange 핸들러에서 setState를 호출하여 값을 업데이트합니다.\n\n리액트가 폼 데이터의 유일한 진실 공급원(single source of truth)이 되므로 입력값 검증, 조건부 제출, 실시간 포맷팅 등 세밀한 제어가 가능합니다.",
  },
  {
    id: 47,
    category: "React",
    ...
    answer:
      "Uncontrolled 패턴은 폼 요소의 값을 DOM 자체가 관리하는 방식으로, 리액트 state 대신 useRef를 사용하여 필요할 때 DOM에서 직접 값을 가져옵니다.\n\nstate 업데이트가 발생하지 않으므로 입력마다 리렌더링이 발생하지 않아 성능상 이점이 있습니다.\n\n단순한 폼이나 파일 입력처럼 리액트로 제어할 필요가 없는 경우에 적합하며, defaultValue 속성으로 초기값을 설정합니다.",
  },
];
