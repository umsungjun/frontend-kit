...
export const react: Flashcard[] = [
  ...
  },
  {
    ...
    category: "React",
    question:
      "리액트에서 함수형 컴포넌트가 아니라 함수 컴포넌트라고 부르는 이유는 뭔가요?",
    answer:
  ...
  },
  {
    ...
    category: "React",
    question:
      "props는 부모에서 자식으로 전달하는데, 자식에서 부모로도 전달할 수 있나요?",
    answer:
      "리액트의 데이터 흐름은 부모에서 자식으로 흐르는 단방향이라, 자식이 직접 부모에게 props를 전달할 수는 없습니다.\n\n대신 부모가 콜백 함수를 props로 내려주고, 자식이 그 함수를 호출하며 인자로 데이터를 넘기면 간접적으로 부모에게 값을 전달할 수 있습니다.\n\n여러 컴포넌트가 공유하는 상태를 부모로 옮겨 이렇게 콜백으로 갱신하는 패턴을 상태 끌어올리기(lifting state up)라고 합니다.",
  },
  {
    id: 13,
    category: "React",
  ...
  },
  {
    id: 16,
    category: "React",
    question: "React에서 state의 불변성을 유지하라는 게 무슨 뜻인가요?",
    answer:
  ...
  },
  {
    ...
    category: "React",
    question:
      "리듀서에서 불변성을 지켜야 하는 이유와 전개 연산자의 단점을 보완하는 방법이 뭔가요?",
    answer:
  ...
  },
  {
    ...
    answer:
      "setState는 호출 즉시 state를 바꾸지 않고, 여러 호출을 모아 다음 렌더에서 한 번에 반영합니다. 그래서 흔히 '비동기적'이라고 표현합니다.\n\n따라서 setState 직후 같은 함수 안에서는 변경된 값을 읽을 수 없습니다. 현재 렌더의 state 값은 클로저에 고정돼 있고, 새 값은 다음 렌더에서야 반영되기 때문입니다.\n\n업데이트된 값을 기준으로 계산하려면 setState에 콜백 형태(prev => newState)를 전달하고, 값이 바뀐 뒤의 후속 처리는 useEffect에서 하면 됩니다.",
  },
  {
    id: 30,
    category: "React",
  ...
  },
  {
    ...
    answer:
      "staleTime은 데이터를 얼마나 오래 '신선한 상태'로 볼지 결정하고, gcTime은 쿼리가 사용되지 않은 뒤 캐시를 메모리에 얼마나 유지할지 결정합니다. 역할이 다른 두 개의 타이머입니다.\n\nstaleTime은 기본값이 0이라 데이터를 가져오는 즉시 stale 상태가 됩니다. 이 값을 늘려두면 그 시간 동안은 refetch 트리거가 발생해도 새 네트워크 요청을 보내지 않고 캐시된 데이터를 그대로 씁니다.\n\ngcTime(이전 이름은 cacheTime)은 기본값이 5분입니다. 컴포넌트가 언마운트되어 해당 쿼리를 구독하는 곳이 없어지면 gcTime 이후에 캐시에서 완전히 제거됩니다. 데이터가 stale 상태여도 gcTime이 남아 있으면 캐시는 유지되므로, 다시 요청이 들어올 때 로딩 없이 기존 데이터를 먼저 보여주면서 백그라운드에서 새로 가져올 수 있습니다.\n\n둘을 함께 조절하면 불필요한 네트워크 요청을 줄이면서도 오래된 캐시가 메모리를 낭비하지 않도록 균형을 맞출 수 있습니다.",
  },
];
