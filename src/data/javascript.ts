...
export const javascript: Flashcard[] = [
  {
    ...
    answer:
      "프로그래밍은 컴퓨터에게 특정 작업을 수행하도록 명령어를 작성하는 과정입니다.\n\n사람의 의도를 프로그래밍 언어라는 도구를 통해 컴퓨터가 이해할 수 있는 형태로 변환하여 문제를 해결하는 행위를 말합니다.\n\n결국 프로그래밍은 문제를 정의하고 이를 코드로 풀어내는 사고 과정 그 자체라고 할 수 있습니다.",
  },
  {
    id: 2,
    category: "JavaScript",
    ...
    answer:
      "컴파일러와 인터프리터는 모두 소스 코드를 기계어로 번역해 실행하지만, 번역 시점과 방식에 차이가 있습니다.\n\n컴파일러는 소스 코드 전체를 한꺼번에 기계어로 번역한 후 실행하므로 실행 속도가 빠르지만 번역 시간이 필요합니다.\n\n인터프리터는 소스 코드를 한 줄씩 읽어가며 즉시 실행하므로 즉시 실행은 가능하지만 실행 속도가 상대적으로 느립니다.\n\n자바스크립트는 기본적으로 인터프리터 언어이지만, V8 같은 현대 엔진은 JIT 컴파일을 사용하여 성능을 최적화합니다.",
  },
  {
    id: 3,
    category: "JavaScript",
    ...
    answer:
      "자바스크립트는 웹 브라우저에서 동작하는 인터프리터 기반의 프로그래밍 언어로, 동적 타이핑을 지원합니다.\n\n프로토타입 기반 객체지향을 지원하며, 일급 함수를 가진 멀티 패러다임 언어입니다.\n또한 싱글 스레드로 동작하지만 이벤트 루프를 통해 비동기 처리를 효율적으로 수행할 수 있습니다.\n\n이러한 특징은 브라우저 환경에서 동적 UI 구현과 비동기 통신을 자연스럽게 다루는 데 적합합니다.",
  },
  {
    id: 4,
    category: "JavaScript",
    question: "변수란 무엇인가요?",
    answer:
      "변수는 하나의 값을 저장하기 위해 확보한 메모리 공간 또는 그 메모리 공간을 식별하기 위해 붙인 이름입니다.\n\n프로그래밍에서 값을 저장하고 참조하기 위한 메커니즘으로, 값의 위치를 가리키는 상징적인 이름이라고 할 수 있습니다.\n\n변수에 의미 있는 이름을 부여하면 코드의 가독성과 유지보수성이 높아집니다.",
  },
  {
    id: 5,
    category: "JavaScript",
    question: "식별자란 무엇인가요?",
    answer:
      "식별자는 변수, 함수, 클래스 등 어떤 값을 구별하여 식별할 수 있는 고유한 이름을 말합니다.\n\n식별자는 값이 아닌 메모리 주소를 기억하고 있으며, 이를 통해 메모리에 저장된 값에 접근할 수 있습니다.\n\n선언에 의해 자바스크립트 엔진에 식별자의 존재를 알리게 됩니다.",
  },
  {
    id: 6,
    category: "JavaScript",
    ...
    answer:
      "변수를 선언한다는 것은 값을 저장하기 위한 메모리 공간을 확보하고, 변수 이름과 해당 메모리 주소를 연결하여 값을 저장할 수 있게 준비하는 것을 의미합니다.\n\n자바스크립트의 변수 선언은 두 단계로 이루어집니다.\n선언 단계에서 변수 이름을 등록하여 자바스크립트 엔진에 알립니다.\n초기화 단계에서 메모리 공간을 확보한 뒤 undefined를 할당합니다.\n\n이러한 과정을 통해 변수는 안전하게 사용할 준비를 마치게 됩니다.",
  },
  {
    id: 7,
    category: "JavaScript",
    ...
    answer:
      "var는 자바스크립트에서 변수를 선언할 때 사용하는 키워드로, ES6 이전에는 유일한 변수 선언 방법이었습니다.\n\nvar로 선언한 변수는 함수 레벨 스코프를 가집니다.\n선언과 초기화가 동시에 이루어져 호이스팅 시 undefined로 초기화됩니다.\n중복 선언이 허용되고, 선언 전에도 참조가 가능합니다.\n\n이러한 특징은 예측하기 어려운 동작을 유발할 수 있어, 현대 자바스크립트에서는 let과 const 사용이 권장됩니다.",
  },
  {
    id: 8,
    category: "JavaScript",
    question: "호이스팅이 뭔가요?",
    answer:
      "호이스팅은 자바스크립트 엔진이 코드 실행 전 변수와 함수 선언을 해당 스코프의 최상단으로 끌어올리는 것처럼 동작하는 특성입니다.\n\nvar는 선언과 초기화가 함께 호이스팅되어 undefined로 접근이 가능합니다.\nlet과 const는 선언만 호이스팅되어 TDZ에 의해 초기화 전 접근 시 에러가 발생합니다.\n함수 선언문은 전체가 호이스팅되어 선언 전에 호출이 가능합니다.\n\n호이스팅의 동작 차이를 이해하면 변수 선언 위치에 따른 버그를 예방할 수 있습니다.",
  },
  {
    id: 9,
    category: "JavaScript",
    ...
    answer:
      "var는 함수 레벨 스코프만 지원하고 중복 선언이 허용되어 안전하지 않은 동작을 유발할 수 있습니다.\n\n블록 내에서 선언해도 블록 외부에서 접근이 가능합니다.\n중복 선언이 허용되어 의도치 않게 변수값이 변경될 수 있습니다.\n호이스팅으로 인해 선언 전에 참조해도 에러가 발생하지 않아 예측하기 어려운 코드가 됩니다.\n\n이러한 문제들 때문에 ES6에서 let과 const가 도입되었습니다.",
  },
  {
    id: 10,
    category: "JavaScript",
    ...
    answer:
      "let과 var는 모두 변수를 선언하는 키워드이지만, 스코프와 중복 선언, 호이스팅 동작에서 차이가 있습니다.\n\nvar는 함수 레벨 스코프를 가지며 중복 선언이 허용되고, 호이스팅 시 undefined로 초기화되어 선언 전에도 참조가 가능합니다.\n\nlet은 블록 레벨 스코프를 가져 if문이나 for문 등의 블록 내에서 선언한 변수는 블록 외부에서 접근할 수 없습니다.\n중복 선언이 불가능하여 같은 스코프에서 같은 이름으로 다시 선언하면 SyntaxError가 발생합니다.\nTDZ가 존재하여 선언 전에 접근하면 ReferenceError가 발생합니다.\n\n예측 가능한 동작을 위해 현대 자바스크립트에서는 var 대신 let과 const 사용이 권장됩니다.",
  },
  {
    id: 11,
    category: "JavaScript",
  ...
  },
  {
    ...
    answer:
      "const는 반드시 선언과 동시에 초기화해야 하며, 재할당이 불가능한 상수를 선언할 때 사용하는 키워드입니다.\n\nlet과 마찬가지로 블록 레벨 스코프를 가지며 중복 선언이 불가능합니다.\n다만 const로 선언한 객체나 배열의 경우, 참조값 자체는 변경할 수 없지만 객체의 프로퍼티나 배열의 요소는 변경할 수 있습니다.\n\n변하지 않는 값을 표현할 때 const를 우선적으로 사용하면 코드의 의도를 명확히 전달할 수 있습니다.",
  },
  {
    id: 13,
    category: "JavaScript",
    question: "리터럴이 뭔가요?",
    answer:
      "리터럴은 사람이 이해할 수 있는 문자 또는 약속된 기호를 사용해 값을 생성하는 표기법입니다.\n\n예를 들어 숫자 리터럴 100, 문자열 리터럴 'hello', 객체 리터럴 {}, 배열 리터럴 [] 등이 있습니다.\n\n자바스크립트 엔진은 런타임에 리터럴을 평가하여 값을 생성합니다.",
  },
  {
    id: 14,
    category: "JavaScript",
  ...
  },
  {
    ...
    answer:
      "데이터 타입은 값을 다룰 때 메모리를 어떻게 할당하고 해석할지 결정하기 위해 필요합니다.\n\n값을 저장할 때 확보해야 하는 메모리 공간의 크기를 결정합니다.\n메모리에서 값을 읽을 때 한 번에 읽어야 할 메모리 공간의 크기를 알기 위해 필요합니다.\n읽어온 2진수를 어떻게 해석할지 결정하기 위해 필요합니다.\n\n타입이 명확해야 자바스크립트 엔진이 값을 올바르게 다룰 수 있습니다.",
  },
  {
    id: 16,
    category: "JavaScript",
  ...
  },
  {
    ...
    answer:
      "동적 타이핑은 변수 선언 시 타입을 지정하지 않고, 할당된 값에 의해 런타임에 타입이 자동으로 결정되는 방식입니다.\n\n자바스크립트는 대표적인 동적 타이핑 언어로, 같은 변수에 다른 타입의 값을 자유롭게 재할당할 수 있습니다.\n\n유연하지만 타입 관련 버그가 발생하기 쉬워 주의가 필요합니다.",
  },
  {
    id: 18,
    category: "JavaScript",
  ...
  },
  {
    id: 21,
    category: "JavaScript",
    question: "고차 함수에 대해 알고 있나요?",
    answer:
      "고차 함수는 함수를 인수로 전달받거나 함수를 반환하는 함수를 말합니다.\n\n자바스크립트의 함수는 일급 객체이므로 값처럼 전달하고 반환할 수 있어 고차 함수를 만들 수 있습니다.\n대표적으로 배열의 map, filter, reduce 등이 고차 함수에 해당합니다.\n\n고차 함수를 활용하면 외부 상태를 변경하지 않는 선언적 프로그래밍이 가능해집니다.",
  },
  {
    id: 22,
    category: "JavaScript",
  ...
  },
  {
    ...
    answer:
      "자바스크립트에서 객체는 키(key)와 값(value)으로 구성된 프로퍼티의 집합입니다.\n\n원시 타입을 제외한 함수, 배열, 정규표현식 등 거의 모든 것이 객체에 해당합니다.\n\n객체는 프로퍼티를 통해 상태(state)를, 메서드를 통해 동작(behavior)을 하나로 묶어 표현할 수 있는 자료구조입니다.",
  },
  {
    id: 24,
    category: "JavaScript",
    ...
    answer:
      "함수와 메서드는 모두 호출 가능한 코드 블록이지만, 객체와의 관계와 this 바인딩 동작이 다릅니다.\n\n함수는 독립적으로 정의되어 호출할 수 있는 코드 블록이며, 일반 함수의 this는 전역 객체(또는 strict 모드에서 undefined)를 가리킵니다.\n\n메서드는 객체의 프로퍼티로 할당된 함수로, 자신이 속한 객체를 this로 참조할 수 있습니다.\nES6의 메서드 축약 표현으로 정의된 메서드는 내부 슬롯 [[HomeObject]]를 가지며, super 키워드를 사용할 수 있습니다.\n\n호출 맥락에 따라 this의 의미가 달라지므로 두 개념을 구분해서 이해하는 것이 중요합니다.",
  },
  {
    id: 25,
    category: "JavaScript",
    ...
    answer:
      "자바스크립트에서는 다양한 방식으로 객체를 생성할 수 있으며, 상황에 따라 적절한 방법을 선택합니다.\n\n객체 리터럴({})을 사용하는 방법이 가장 일반적입니다.\nObject 생성자 함수(new Object()), 생성자 함수, Object.create() 메서드를 사용할 수 있습니다.\nES6의 클래스(class)를 사용하는 방법도 있습니다.\n\n객체 리터럴은 간편하지만 동일한 구조의 객체를 여러 개 만들 때는 비효율적이며, 이런 경우 생성자 함수나 클래스를 사용하는 것이 적합합니다.",
  },
  {
    id: 26,
    category: "JavaScript",
    ...
    answer:
      "객체에 프로퍼티로 접근하면, 자바스크립트 엔진은 먼저 해당 객체의 메모리 주소를 찾고 그 안에서 프로퍼티 키를 검색합니다.\n\n프로퍼티 값이 원시 타입이면 해당 값이 직접 저장되어 있고, 참조 타입이면 또 다른 메모리 주소를 참조합니다.\n\nV8 같은 엔진은 히든 클래스(Hidden Class)와 인라인 캐싱 등의 최적화 기법으로 프로퍼티 접근 성능을 향상시킵니다.",
  },
  {
    id: 27,
    category: "JavaScript",
  ...
  },
  {
    ...
    answer:
      "값에 의한 전달은 원시 타입의 값을 다른 변수에 할당할 때 원본 값이 복사되어 전달되는 방식입니다.\n\n복사된 값은 원본과 독립적인 메모리 공간에 저장되므로, 한쪽 값을 변경해도 다른 쪽에 영향을 주지 않습니다.\n\n원시 값의 불변성(immutability)을 보장하는 기본 동작 방식입니다.",
  },
  {
    id: 29,
    category: "JavaScript",
    ...
    answer:
      "참조에 의한 전달은 객체를 변수에 할당할 때 메모리 주소(참조값)가 복사되어 전달되는 방식입니다.\n\n두 변수가 같은 객체를 가리키므로, 한쪽에서 객체를 수정하면 다른 변수를 통해서도 변경된 내용이 반영됩니다.\n\n이로 인해 의도치 않은 부수 효과가 발생할 수 있어, 불변 객체를 만들거나 깊은 복사를 활용하는 것이 중요합니다.",
  },
  {
    id: 30,
    category: "JavaScript",
    ...
    answer:
      "함수 선언문과 함수 표현식은 함수를 정의하는 방식과 호이스팅 동작에서 차이가 있습니다.\n\n함수 선언문은 function 키워드로 함수를 정의하며 함수 이름을 생략할 수 없습니다. 전체가 호이스팅되어 선언 전에 호출이 가능합니다.\n\n함수 표현식은 변수에 함수를 할당하는 방식입니다. 변수 호이스팅만 발생하여 선언 전에 호출하면 에러가 발생합니다.\n\n호이스팅 동작 차이를 이해하면 함수 선언 위치를 결정하는 데 도움이 됩니다.",
  },
  {
    id: 31,
    category: "JavaScript",
  ...
  },
  {
    ...
    question: "스코프가 뭔가요?",
    answer:
      "스코프는 식별자(변수, 함수, 클래스 등)가 유효한 범위를 말합니다.\n\n자바스크립트 엔진이 식별자를 검색할 때 사용하는 규칙이기도 합니다.\n\n스코프는 계층적으로 연결되어 스코프 체인을 형성하며, 변수를 참조할 때 현재 스코프에서 상위 스코프 방향으로 이동하며 검색합니다.",
  },
  {
    id: 33,
    category: "JavaScript",
    ...
    answer:
      "스코프는 크게 전역 스코프와 지역 스코프로 나뉩니다.\n\n전역 스코프는 코드의 가장 바깥 영역으로 어디서든 참조할 수 있습니다.\n지역 스코프는 함수 몸체 내부를 말합니다.\n또한 var는 함수 레벨 스코프만 인정하고, let과 const는 블록 레벨 스코프를 가져 if, for, while 등의 코드 블록 내에서 유효합니다.\n\n선언 키워드에 따라 적용되는 스코프 단위가 달라지므로, 변수의 유효 범위를 의식하며 코드를 작성해야 합니다.",
  },
  {
    id: 34,
    category: "JavaScript",
  ...
  },
  {
    ...
    answer:
      "전역 변수는 어디서든 참조 가능하여 의도치 않게 값이 변경될 위험이 있고, 생명 주기가 길어 메모리 리소스를 오래 소비합니다.\n\n스코프 체인의 종점에 위치하여 변수 검색 속도가 느립니다.\n다른 파일의 전역 변수와 이름이 충돌할 수 있습니다.\n\n이를 방지하기 위해 즉시 실행 함수, 모듈 패턴, ES6 모듈 등을 활용하는 것이 좋습니다.",
  },
  {
    id: 36,
    category: "JavaScript",
  ...
  },
  {
    ...
    question: "일급 객체가 뭔가요?",
    answer:
      "일급 객체란 변수에 할당할 수 있고, 함수의 인수로 전달할 수 있으며, 함수의 반환값으로 사용할 수 있는 객체를 말합니다.\n\n또한 런타임에 생성이 가능하고, 무명 리터럴로 생성할 수 있어야 합니다.\n\n자바스크립트의 함수는 이 모든 조건을 충족하므로 일급 객체에 해당합니다.",
  },
  {
    id: 40,
    category: "JavaScript",
    ...
    answer:
      "함수가 일급 객체이기 때문에 값처럼 자유롭게 다룰 수 있고, 다양한 프로그래밍 패턴을 구현할 수 있습니다.\n\n함수를 변수에 저장하거나 데이터 구조(배열, 객체)에 담을 수 있습니다.\n다른 함수의 인수로 전달하여 콜백 패턴을 구현할 수 있습니다.\n함수에서 함수를 반환하여 클로저를 만들거나, 고차 함수를 구성할 수 있습니다.\n\n이를 통해 함수형 프로그래밍 패턴, 커링, 데코레이터 패턴 등 다양한 프로그래밍 기법을 활용할 수 있습니다.",
  },
  {
    id: 41,
    category: "JavaScript",
  ...
  },
  {
    ...
    answer:
      "this 바인딩은 this가 가리킬 객체를 연결하는 것을 말합니다.\n\n자바스크립트에서 this 바인딩은 함수 호출 시점에 동적으로 결정됩니다.\n일반 함수 호출에서는 전역 객체, 메서드 호출에서는 호출한 객체, 생성자 함수에서는 생성될 인스턴스, call/apply/bind에서는 명시적으로 지정한 객체에 바인딩됩니다.\n\n호출 방식에 따라 this가 달라지므로 코드를 작성할 때 호출 컨텍스트를 의식해야 합니다.",
  },
  {
    id: 52,
    category: "JavaScript",
    ...
    answer:
      "this는 함수의 호출 방식에 따라 서로 다른 객체에 바인딩됩니다.\n\n일반 함수로 호출하면 this는 전역 객체(window 또는 global)에 바인딩되고, strict 모드에서는 undefined입니다.\n메서드로 호출하면 호출한 객체에 바인딩되고, 생성자 함수로 호출하면 새로 생성되는 인스턴스에 바인딩됩니다.\ncall, apply, bind 메서드를 사용하면 this를 명시적으로 지정할 수 있습니다.\n화살표 함수는 자체 this가 없어 상위 스코프의 this를 그대로 사용합니다.\n\n호출 방식별 this 바인딩 규칙을 이해하면 의도치 않은 this 동작을 예방할 수 있습니다.",
  },
  {
    id: 53,
    category: "JavaScript",
  ...
  },
  {
    ...
    answer:
      "에러 처리를 하지 않으면 예외 발생 시 프로그램이 강제로 종료되어 사용자 경험이 크게 저하됩니다.\n\n적절한 에러 처리를 통해 프로그램의 안정성을 높이고, 사용자에게 의미 있는 피드백을 제공할 수 있습니다.\n\n또한 에러의 원인을 로깅하여 디버깅을 쉽게 만들고, 예외 상황에서도 프로그램이 정상적으로 동작하도록 보장할 수 있습니다.",
  },
  {
    id: 81,
    category: "JavaScript",
