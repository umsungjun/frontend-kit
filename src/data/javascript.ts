...
export const javascript: Flashcard[] = [
  ...
  },
  {
    ...
    question: "일급 객체가 뭔가요?",
    answer:
      "일급 객체란 변수에 할당할 수 있고, 함수의 인수로 전달할 수 있으며, 함수의 반환값으로 사용할 수 있는 객체를 말합니다.\n\n여기에 런타임 생성과 무명 리터럴 생성까지 가능해야 하는데, 자바스크립트의 함수는 이 조건을 모두 충족해 일급 객체입니다.\n\n덕분에 함수를 값처럼 다뤄 콜백으로 전달하거나, 함수를 반환해 클로저·고차 함수를 만드는 등 함수형 프로그래밍 기법을 자유롭게 쓸 수 있습니다.",
  },
  {
    id: 41,
    category: "JavaScript",
  ...
  },
  {
    ...
    question: "순수 함수가 뭔가요?",
    answer:
      "순수 함수는 동일한 인수에 대해 항상 동일한 결과를 반환하고, 외부 상태를 변경하지 않는(부수 효과가 없는) 함수입니다.\n\n함수의 실행이 외부 변수나 상태에 의존하지 않으며, 외부 상태를 수정하지도 않습니다.\n\n이러한 특성 덕분에 코드의 예측 가능성이 높아지고 테스트가 쉬워집니다.",
  },
  {
    id: 43,
    category: "JavaScript",
  ...
  },
  {
    ...
    answer:
      "객체지향 프로그래밍(OOP)은 프로그램을 독립적인 객체의 집합으로 구성하여, 객체 간의 상호작용으로 프로그램을 설계하는 패러다임입니다.\n\n실세계의 사물이나 개념을 속성(프로퍼티)과 행위(메서드)를 가진 객체로 모델링합니다.\n\n코드의 재사용성, 유지보수성, 확장성을 높이는 것이 주요 목적입니다.",
  },
  {
    id: 45,
    category: "JavaScript",
    ...
    answer:
      "객체지향 프로그래밍의 4가지 핵심 특징은 캡슐화, 상속, 다형성, 추상화입니다.\n\n캡슐화는 데이터와 메서드를 하나로 묶고 외부 접근을 제어합니다.\n상속은 기존 객체의 속성과 메서드를 물려받아 재사용합니다.\n다형성은 같은 인터페이스로 다양한 동작을 구현하는 것입니다.\n추상화는 복잡한 내부 구현을 숨기고 필요한 부분만 노출하는 것입니다.\n\n이 네 가지 특징이 어우러져 유연하고 확장 가능한 객체지향 설계를 가능하게 합니다.",
  },
  {
    id: 46,
    category: "JavaScript",
    ...
    answer:
      "자바스크립트는 프로토타입 기반의 객체지향 프로그래밍을 지원하는 멀티 패러다임 언어입니다.\n\n클래스 기반 언어(Java, C++)와 달리 프로토타입 체인을 통해 상속을 구현하며, ES6부터는 class 문법을 제공하지만 이는 프로토타입의 문법적 설탕(syntactic sugar)입니다.\n\n객체지향뿐만 아니라 함수형 프로그래밍, 명령형 프로그래밍도 지원합니다.",
  },
  {
    id: 47,
    category: "JavaScript",
