...
export const typescript: Flashcard[] = [
  ...
  },
  {
    ...
    answer:
      "interface와 type 모두 객체 타입을 정의할 때 사용할 수 있지만, 동작 방식에 차이가 있습니다.\n\ninterface의 가장 큰 특징은 선언 병합입니다. 동일한 이름으로 여러 번 선언하면 자동으로 합쳐지는데, 외부 라이브러리의 타입을 확장하거나 전역 타입을 보강할 때 유용합니다. 객체 타입 확장에 특화되어 있고 extends 키워드로 상속할 수 있습니다.\n\ntype은 유니온, 인터섹션, 튜플, 원시 타입 등 더 다양한 타입 표현이 가능합니다. 인터섹션(&)과 유니온(|) 연산자를 자유롭게 조합할 수 있어 복잡한 타입을 만들 때 적합합니다. 다만 중복 선언은 허용되지 않습니다.\n\n실무에서는 Props나 API 응답처럼 외부와의 계약 성격을 가진 타입에는 interface를, 유니온이나 유틸리티 타입 조합처럼 내부 로직에서 활용하는 타입에는 type을 쓰는 컨벤션이 많이 사용됩니다.",
  },
  {
    id: 7,
    category: "TypeScript",
  ...
  },
  {
    id: 25,
    category: "TypeScript",
    question: "TypeScript의 infer 키워드가 뭔가요?",
    answer:
      "infer는 조건부 타입 안에서 특정 타입을 추론하도록 TypeScript에게 위임할 때 사용하는 키워드입니다. 타입을 직접 지정하는 것이 아니라 컴파일러가 해당 위치의 타입을 유추하게 하는 방식입니다.\n\ninfer는 반드시 extends를 사용하는 조건부 타입 안에서만 쓸 수 있습니다. 예를 들어 `T extends (...args: any[]) => infer R ? R : never`라고 작성하면, 함수 타입 T의 반환 타입을 R로 추론해 꺼낼 수 있습니다. TypeScript 내장 유틸리티 타입인 `ReturnType<T>`도 이 패턴으로 구현되어 있습니다.\n\n실용적인 활용 사례는 여럿 있습니다. `Promise<T>`에서 T를 추출하거나, 배열 타입에서 요소 타입을 꺼내거나, 함수의 매개변수 타입을 추론하는 등 복잡한 타입 변환을 깔끔하게 표현할 수 있습니다.\n\n처음에는 다소 낯설게 느껴지지만, 조건부 타입과 infer를 익히면 기존 타입을 재활용하는 유연한 타입 유틸리티를 직접 만들 수 있습니다.",
  },
  {
    id: 26,
    category: "TypeScript",
    question: "TypeScript의 satisfies 키워드가 뭔가요?",
    answer:
      "satisfies는 TypeScript 4.9에서 추가된 키워드로, 값의 구체적인 타입 정보를 유지하면서 동시에 특정 타입 조건을 충족하는지 검사하고 싶을 때 사용합니다.\n\n예를 들어 `Record<Color, string | RGB>`로 타입을 명시하면 palette.green의 타입이 string | RGB로 넓어져서 .toUpperCase()를 바로 호출할 수 없습니다. 반면 satisfies `Record<Color, string | RGB>`를 사용하면 palette.green이 string으로 좁게 추론된 채 유지되어 메서드를 안전하게 호출할 수 있습니다.\n\n핵심 차이는 타입 애너테이션(: Type)이 타입을 지정해서 구체적인 정보를 덮어버리는 반면, satisfies는 타입 조건만 검증하고 추론된 구체적인 타입은 그대로 보존한다는 점입니다.\n\n유니온 타입을 다루는 객체에서 각 값의 실제 타입을 살려두고 싶을 때, 또는 객체 리터럴이 필수 필드를 모두 갖추고 있는지 검사하면서도 추가 필드의 타입까지 정확히 쓰고 싶을 때 유용합니다.",
  },
];
